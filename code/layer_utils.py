...
def affine_batchnorm_relu_forward(x, w, b, gamma, beta, bn_param):
    a, fc_cache = affine_forward(x, w, b)
    b_out, batch_cache = batchnorm_forward(a, gamma, beta, bn_param)
...
def affine_batchnorm_relu_backward(dout, cache):
    ...
    dbatchnorm = relu_backward(dout, relu_cache)
    da, dgamma, dbeta = batchnorm_backward(dbatchnorm, batch_cache)
    dx, dw, db = affine_backward(da, fc_cache)
...
def conv_batch_relu_pool_forward(x, w, b, conv_param, pool_param, gamma, beta, bn_param):
  a, conv_cache = conv_forward_fast(x, w, b, conv_param)
  b_out, batch_cache = spatial_batchnorm_forward(a, gamma, beta, bn_param)
  s, relu_cache = relu_forward(b_out)
  out, pool_cache = max_pool_forward_fast(s, pool_param)
...
def conv_batch_relu_pool_backward(dout, cache):
  ...
  ds = max_pool_backward_fast(dout, pool_cache)
  dbatchnorm = relu_backward(ds, relu_cache)
  da, dgamma, dbeta = spatial_batchnorm_backward(dbatchnorm, batch_cache)
  dx, dw, db = conv_backward_fast(da, conv_cache)
  return dx, dw, db, dgamma, dbeta